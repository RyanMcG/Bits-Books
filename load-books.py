from bitslib.models import Book as b, Author as a, Book_Author as ba, Publisher as p, Category as c, Book_Category as bc
...
class BookLoader():
	def load(self):
		...
		for bookentry in booklist:
				...
				new_publisher = p(publisher, city, states[city], 'United States', est_date)
				...
				if len(bookentry[0]) > 0:
					...
					for bo in b.query.all():
						if bo.title == bookentry[1]:
							previous_book = bo.id
					ba_stmnt = ba.insert().values(book_id=previous_book, author_id=auth_id, date_created=est_date, date_modified=est_date)
					bc_stmnt = bc.insert().values(book_id=previous_book, category_id=cat_id, date_created=est_date, date_modified=est_date)
					db.engine.execute(ba_stmnt)
					db.engine.execute(bc_stmnt)
					db.session.commit()	
					
					# Create inventory
					new_inventory = i(previous_book, random.randrange(1,100), 'OK')
